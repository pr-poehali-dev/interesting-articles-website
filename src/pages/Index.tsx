...
const Index = () => {
  ...
  const [articlesRef, articlesVisible, visibleArticles] = useStaggeredAnimation(6, 150);
  const [aboutRef, aboutVisible] = useScrollAnimation();
  const [commentsRef, commentsVisible] = useScrollAnimation();
  const [contactsRef, contactsVisible] = useScrollAnimation();
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-white">
      ...
      <section ref={heroRef} className="relative py-20 overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center relative z-10">
          <h2 className={`text-5xl md:text-6xl font-bold mb-6 bg-gradient-to-r from-[#FF6B6B] to-[#4ECDC4] bg-clip-text text-transparent transition-all duration-1000 ${
            heroVisible ? 'animate-fade-in opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}>
            Интересные статьи
          </h2>
          <p className={`text-xl text-gray-600 mb-8 max-w-2xl mx-auto transition-all duration-1000 delay-300 ${
          ...
          {/* Search */}
          <div className={`max-w-md mx-auto mb-12 transition-all duration-1000 delay-600 ${
      ...
      {/* Categories */}
      <section ref={categoriesRef} id="categories" className="py-12 bg-white">
        <div className="container mx-auto px-4">
          <h3 className={`text-3xl font-bold text-center mb-8 transition-all duration-800 ${
          ...
          <div className={`flex flex-wrap justify-center gap-4 transition-all duration-800 delay-300 ${
            ...
            {categories.map((category, index) => (
              <Badge
                ...
                className={`px-6 py-2 text-sm font-medium cursor-pointer transition-all hover-scale duration-300 ${
      ...
      {/* Articles */}
      <section ref={articlesRef} id="articles" className="py-16">
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredArticles.length > 0 ? (
              filteredArticles.map((article, index) => (
                <Card 
                  key={article.id} 
                  className={`overflow-hidden hover-scale transition-all duration-500 border-0 shadow-lg hover:shadow-xl ${
                    visibleArticles.has(index) ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
                  }`}
                >
      ...
      {/* Comments Section */}
      <section ref={commentsRef} className="py-16 bg-gray-50">
        <div className="container mx-auto px-4 max-w-4xl">
          <h3 className={`text-3xl font-bold text-center mb-12 transition-all duration-800 ${
            commentsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}>Обсуждение</h3>
          
          {/* Add Comment */}
          <Card className={`mb-8 transition-all duration-800 delay-300 ${
            commentsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}>
            <CardHeader>
      ...
      <section ref={aboutRef} id="about" className="py-16 bg-white">
        <div className="container mx-auto px-4 text-center">
          <h3 className={`text-3xl font-bold mb-8 transition-all duration-800 ${
            aboutVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}>О блоге</h3>
          <div className="max-w-3xl mx-auto">
            <p className={`text-lg text-gray-600 mb-6 transition-all duration-800 delay-300 ${
              aboutVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
            }`}>
            ...
            <div className={`grid grid-cols-1 md:grid-cols-3 gap-8 mt-12 transition-all duration-800 delay-600 ${
              aboutVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
            }`}>
      ...
      {/* Contacts */}
      <section ref={contactsRef} id="contacts" className="py-16 bg-gray-50">
        <div className="container mx-auto px-4 text-center">
          <h3 className={`text-3xl font-bold mb-8 transition-all duration-800 ${
            contactsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
          }`}>Контакты</h3>
          <div className="max-w-md mx-auto">
            <Card className={`transition-all duration-800 delay-300 ${
              contactsVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'
            }`}>
              <CardContent className="pt-6">
                <div className="space-y-4">
